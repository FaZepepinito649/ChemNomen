...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    const board = Object.entries(users)
      .map(([name, d]) => ({ name, score: d.score || 0, correct: d.correct || 0, total: d.total || 0 }))
  ...
  if (event.httpMethod === 'POST') {
    ...
    if (username === 'admin') {
      // Admin progress not persisted (they're just testing)
      return json(200, { score: 0, correct: 0, total: 0 });
    }
    ...
    return json(200, { score: u.score, correct: u.correct, total: u.total, categoryStats: u.categoryStats });
  }
